...
module.exports.updateProfile = async (req, res, next) => {
  try {
    ...
    const profilePath = req.files.profile ? `images/profiles/${req.files.profile[0].filename}` : `${currentUser.profilePicture}`;

    const coverPath = req.files.cover ? `images/profiles/${req.files.cover[0].filename}` : `${currentUser.cover}`;
